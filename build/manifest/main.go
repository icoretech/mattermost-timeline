...
const manifestVersion = %q // x-release-please-version
...
func applyManifest(manifest *model.Manifest) error {
	manifestWithPlaceholder := *manifest
	manifestWithPlaceholder.Version = "__RELEASE_PLEASE_VERSION__"

	if manifest.HasServer() {
		// generate JSON representation of Manifest.
		manifestBytes, err := json.MarshalIndent(&manifestWithPlaceholder, "", "  ")
		if err != nil {
			return err
		}
		...
		if err := os.WriteFile(
			"server/manifest.go",
			fmt.Appendf(nil, pluginIDGoFileTemplate, manifest.Version, manifestStr),
			0o600,
		); err != nil {
	...
	if manifest.HasWebapp() {
		...
		if err := os.WriteFile(
			...
			fmt.Appendf(nil, pluginIDJSFileTemplate, manifest.Version, manifestStr),
			0o600,
		); err != nil {
